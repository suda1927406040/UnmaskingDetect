...
from utils.file_utils import read_dict_file, write_dict_file, readlines, read_file, read_tsv, read_jsonl, read_poison_jsonl, read_json
import config
import glob
...
class TrainDataset():
    ...
    def process_jsonl_data(self):        
        ...
        idxs = random.sample(range(len(all_codes)), 40000)
        codes = [all_codes[idx] for idx in idxs]
        # codes = [code for _, code in read_jsonl(path)]
    ...
    def process_tsv_data(self):
        logger.info(f"==> Tokenize data using {self.tokenizer.name}")
        path = self.data_dir
        f = open(self.txt_path, "w")
        # g = open(self.id2idx_path, "w")
        new_sent_list = read_tsv(path)
        for i, code in new_sent_list:
            tokens = self.tokenizer.tokenize(code)
            self.train_methods.append((i, tokens))
            f.write(" ".join(tokens) + "\n")
...
class TestDataset():
    ...
    def process_poison_jsonl_data(self):
        logger.info(f"==> Tokenize data using {self.tokenizer.name}")
        path = self.data_dir
        f = open(self.txt_path, "w")
        new_sent_list = read_poison_jsonl(path)
        for i, code in new_sent_list:
            tokens = self.tokenizer.tokenize(code)
            self.test_methods.append((i, tokens))
            f.write(" ".join(tokens) + "\n")
